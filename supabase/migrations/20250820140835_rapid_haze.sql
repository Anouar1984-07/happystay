...
BEGIN
    ...
    PERFORM * FROM get_available_slots(CURRENT_DATE);
    RAISE NOTICE 'Fonction get_available_slots créée et testée avec succès';
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Erreur lors du test de la fonction: %', SQLERRM;
END $$;

-- Afficher un exemple de résultat pour vérification
SELECT 'Exemple de résultat pour aujourd''hui:' as message;
